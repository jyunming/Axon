"""Write the complete bench_stores.py from scratch."""
...
RECALL_QUERIES = [
    ...
    },
    {
        ...
        "relevant": ["Mack_2013_Stochastic", "Mack_2018_Shot_Noise",
                     "Mack_2019_Metrics_Stochastic", "Mack_2019_Will_Stochastics"],
    },
    {
...
def run_benchmark(cfg_spec, store_base, papers_dir, gemini_api_key):
    ...
    print(f"  Ingest: {ingest_time:.1f}s  |  {chunk_count} chunks  |  "
...
def print_report(results):
    ...
    for i, q in enumerate(RECALL_QUERIES):
        cells = "".join(
            f"  {r.latencies_ms[i]:5.0f}ms/{r.rr_scores[i]:.2f}"
            for r in results
        )
...
dest = Path(__file__).parent / "bench_stores.py"

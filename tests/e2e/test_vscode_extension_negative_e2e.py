...
Coverage matrix
---------------
NEG-001  API unreachable — axon_searchKnowledge
NEG-002  API unreachable — axon_queryKnowledge
NEG-003  400 bad request — axon_createProject (invalid name)
NEG-004  403 read-only rejection — axon_ingestText
NEG-005  403 read-only rejection — axon_clearCollection
NEG-006  403 read-only rejection — axon_finalizeGraph
NEG-007  404 missing job — axon_getIngestStatus
NEG-008  404 missing project — axon_switchProject
NEG-009  409 project mismatch — axon_queryKnowledge with project param
NEG-010  unsupported image extension — axon_ingestImage (.gif)
NEG-011  no Copilot model available — axon_ingestImage
NEG-012  empty Copilot image description — axon_ingestImage
NEG-013  graph endpoint failure — axon_showGraphStatus 500
NEG-014  store not active — axon_shareProject 404
...
def test_search_api_unreachable(run_tool, extension_js_path, extension_root_path, runner_js_path):
    """NEG-001: axon_searchKnowledge surfaces a connection-error message."""
    ...
    res = helpers.run_extension_tool(
        ...
        tool_name="axon_searchKnowledge",
...
def test_query_api_unreachable(run_tool, extension_js_path, extension_root_path, runner_js_path):
    """NEG-002: axon_queryKnowledge surfaces a connection-error message."""
    import importlib.util
    from pathlib import Path
...
def test_create_project_bad_name_400(run_tool, live_recorder_server):
    """NEG-003: axon_createProject propagates the 400 API error text to the user."""
    base_url, _ = live_recorder_server

    with _error_server(400, "Invalid project name: must not contain spaces") as err_url:
        res = run_tool(err_url, "axon_createProject", {"name": "bad name"})
...
def test_ingest_text_403_mounted(run_tool, live_recorder_server):
    """NEG-004: axon_ingestText on a mounted/read-only project shows 403 error."""
    with _error_server(403, "Write to mounted project not allowed") as err_url:
        res = run_tool(err_url, "axon_ingestText", {"text": "hello"})
    _assert_error_text(res, "403", "error", "not allowed", "mounted")


def test_clear_collection_403_mounted(run_tool, live_recorder_server):
    """NEG-005: axon_clearCollection on a read-only project shows 403 error."""
    with _error_server(403, "Write to mounted project not allowed") as err_url:
        res = run_tool(err_url, "axon_clearCollection", {})
    _assert_error_text(res, "403", "error", "not allowed", "mounted")


def test_finalize_graph_403_mounted(run_tool, live_recorder_server):
    """NEG-006: axon_finalizeGraph on a read-only project shows 403 error."""
    with _error_server(403, "Write to mounted project not allowed") as err_url:
        res = run_tool(err_url, "axon_finalizeGraph", {})
...
def test_get_ingest_status_404_missing_job(run_tool, live_recorder_server):
    """NEG-007: axon_getIngestStatus with an unknown job_id surfaces 'not found'."""
    with _error_server(404, "Job not found") as err_url:
        res = run_tool(err_url, "axon_getIngestStatus", {"job_id": "ghost-job"})
...
def test_switch_project_404_missing(run_tool, live_recorder_server):
    """NEG-008: axon_switchProject to a non-existent project surfaces a 404 error."""
    with _error_server(404, "Project 'ghost' not found") as err_url:
        res = run_tool(err_url, "axon_switchProject", {"name": "ghost"})
...
def test_query_409_project_mismatch(run_tool, live_recorder_server):
    """NEG-009: axon_queryKnowledge with an active different project returns 409 error text."""
    with _error_server(409, "Active project is 'default', requested 'research'") as err_url:
        res = run_tool(err_url, "axon_queryKnowledge", {"query": "test", "project": "research"})
    _assert_error_text(res, "409", "error", "project")


# ---------------------------------------------------------------------------
# NEG-010 / NEG-011 / NEG-012: axon_ingestImage failure modes
# ---------------------------------------------------------------------------
...
def test_ingest_image_unsupported_extension(run_tool, live_recorder_server, tmp_path):
    ...
    res = run_tool(base_url, "axon_ingestImage", {"imagePath": str(gif)})
...
def test_show_graph_status_500(run_tool, live_recorder_server):
    """NEG-013: axon_showGraphStatus with a 500 backend error surfaces the error text."""
    with _error_server(500, "Internal server error") as err_url:
        res = run_tool(err_url, "axon_showGraphStatus", {})
...
def test_share_project_store_not_active_404(run_tool, live_recorder_server):
    """NEG-014: axon_shareProject when AxonStore is not initialised returns 404 error."""
    with _error_server(404, "AxonStore not initialised") as err_url:
        res = run_tool(err_url, "axon_shareProject", {"project": "default", "grantee": "bob"})

...
class TestResultPostprocess:
    RESULTS = [
        {"id": "a", "score": 0.4, "vector_score": 0.4, "text": "alpha"},
        {"id": "b", "score": 0.9, "vector_score": 0.1, "text": "beta"},
        {"id": "a", "score": 0.7, "vector_score": 0.7, "text": "alpha improved"},
    ]

    def test_dedupe_best_by_id_keeps_best_payload_in_first_seen_order(self):
        out = axon_rust.dedupe_best_by_id(self.RESULTS)
        assert [item["id"] for item in out] == ["a", "b"]
        assert out[0]["text"] == "alpha improved"
        assert out[0]["score"] == pytest.approx(0.7)

    def test_filter_results_by_threshold_respects_fused_only_and_fallback_field(self):
        results = [
            {"id": "vec-good", "score": 0.2, "vector_score": 0.8},
            {"id": "bm25-only", "score": 0.0, "vector_score": 0.0, "fused_only": True},
            {"id": "weak", "score": 0.2, "vector_score": 0.1},
        ]
        out = axon_rust.filter_results_by_threshold(results, 0.3, "vector_score")
        assert [item["id"] for item in out] == ["vec-good", "bm25-only"]

    def test_bridge_result_postprocess_parity(self):
        bridge = get_rust_bridge()
        assert bridge.can_result_postprocess()
        deduped = bridge.dedupe_best_by_id(self.RESULTS)
        assert deduped is not None
        assert [item["id"] for item in deduped] == ["a", "b"]
        filtered = bridge.filter_results_by_threshold(
            [
                {"id": "doc", "score": 0.6, "vector_score": 0.1},
                {"id": "weak", "score": 0.2, "vector_score": 0.1},
            ],
            0.3,
            "score",
        ...
        assert [item["id"] for item in filtered] == ["doc"]

...
Demonstrates a planner-critic loop over the Local RAG Brain API:
  1. Check health
...
def check_health(client: httpx.Client) -> bool:
    ...
    try:
        ...
        print(
            "❌  Cannot connect to the Local RAG Brain API at "
            f"{API_URL}.\n"
            "    Start the server with:  uvicorn rag_brain.api:app --reload"
        )
        return False
    ...
    with httpx.Client() as client:
        ...
        if not knowledge_ok:
            ...
            fact = fallback_fact or (
                f"The Local RAG Brain is a fully local, open-source RAG system "
            ...
            inject_knowledge(
                client,
                fact=fact,
                metadata={"source": "agent_orchestration_example", "topic": "rag_brain"},
            )
...
if __name__ == "__main__":
    ...
    demo_question = "What makes the Local RAG Brain different from cloud-based RAG systems?"
    ...
    demo_fallback_fact = (
        "Local RAG Brain runs entirely on-premises: all embeddings, vector "
